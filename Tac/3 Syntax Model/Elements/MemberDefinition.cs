...
namespace Tac.Semantic_Model
{
    ...
    public class ParameterPopulateScope : IPopulateScope<MemberDefinition>
    {
        private readonly string memberName;
        private readonly bool isReadonly;
        private readonly NameKey typeName;
        private readonly Func<bool, NameKey, IBox<IReturnable>, MemberDefinition> make;
        private readonly Box<IReturnable> box = new Box<IReturnable>();

        public ParameterPopulateScope(string item, bool v, NameKey typeToken, Func<bool, NameKey, IBox<IReturnable>, MemberDefinition> make)
        {
            memberName = item ?? throw new ArgumentNullException(nameof(item));
            isReadonly = v;
            typeName = typeToken ?? throw new ArgumentNullException(nameof(typeToken));
        ...
        public IResolveReference<MemberDefinition> Run(IPopulateScopeContext context)
        {
            ...
            return new ParameterResolveReferance(memberName, memberDef, isReadonly, typeName, make, box);
        }
    ...
    public class ParameterResolveReferance : IResolveReference<MemberDefinition>
    {
        private readonly string memberName;
        private readonly Box<MemberDefinition> memberDef;
        private readonly bool isReadonly;
        ...
        public ParameterResolveReferance(
            string memberName,
            Box<MemberDefinition> memberDef,
            bool isReadonly,
            NameKey explicitTypeName,
            Func<bool, NameKey, IBox<IReturnable>, MemberDefinition> make,
            Box<IReturnable> box)
        {
            this.memberName = memberName ?? throw new ArgumentNullException(nameof(memberName));
            this.memberDef = memberDef ?? throw new ArgumentNullException(nameof(memberDef));
        ...
        {
            return box.Fill(make(isReadonly, new NameKey(memberName), context.GetTypeDefintion(typeName)));
        }
    }
}