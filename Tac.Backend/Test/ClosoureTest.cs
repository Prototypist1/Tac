...
namespace Tac.Backend.Test
{
    public class ClosoureTest
    {
        ...
        public void Test()
        {
            ...
            Assert.False(method.Invoke(TypeManager.Member(TypeManager.Double(1))).IsReturn(out var _, out var innerMethod));

            Assert.False(innerMethod.Value.Invoke(TypeManager.Member<IBoxedDouble>(TypeManager.Double(2))).IsReturn(out var _, out var res1));
            Assert.Equal(3, res1.Value.Value);

            Assert.False(innerMethod.Value.Invoke(TypeManager.Member<IBoxedDouble>(TypeManager.Double(3))).IsReturn(out var _, out var res2));
            Assert.Equal(6, res2.Value.Value);

            Assert.False(innerMethod.Value.Invoke(TypeManager.Member<IBoxedDouble>(TypeManager.Double(4))).IsReturn(out var _, out var res3));
