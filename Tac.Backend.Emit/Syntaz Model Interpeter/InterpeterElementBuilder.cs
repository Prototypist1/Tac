...
namespace Tac.Backend.Emit.SyntaxModel
{
    ...
    internal class AssemblyContextWithGenerator: AssemblyContext
    {
    ...
    internal class AssemblyContext
    {
        ...
        private AssemblyContext(IInterpetedScope[] scopes)
        {
        ...
        public AssemblyContext Child(IInterpetedScope scope)
        {
            var scopes = new List<IInterpetedScope> { scope };
            scopes.AddRange(Scopes);
            return new AssemblyContext(scopes.ToArray());
        }

        public static AssemblyContext Root()
        {
            return new AssemblyContext(new IInterpetedScope[] { TypeManager.InstanceScope() });
        }
    ...
    internal interface IAssembledOperation
    {
        void Assemble(AssemblyContext interpetedContext);
    }

    internal interface IAssembledOperationRequiresGenerator {

        void Assemble(AssemblyContextWithGenerator interpetedContext);
    }
