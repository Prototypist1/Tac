        ...
        public override IEnumerable<IError> Validate()
        {
            ...
            if (leftList.Length == rightList.Length)
            {
                foreach (var error in leftList.Zip(rightList, (x, y) => {
                    ...
                    {
                        return Possibly.Is(Error.Other($"cannot subtract {leftReturns} to {rightReturns}"));
                    }
    ...
    internal class SubtractOperationMaker : BinaryOperationMaker<WeakSubtractOperation,ISubtractOperation>
    {
        public SubtractOperationMaker() : base(SymbolsRegistry.StaticSubtractSymbol, (l,r)=>
           new Box<WeakSubtractOperation>(
                new WeakSubtractOperation(l,r)), (s, c, l, r) => OrType.Make<Tpn.IValue, IError>(c.TypeProblem.CreateValue(s, new NameKey("number"), new PlaceholderValueConverter())))
        {}
    }
}
