...
namespace Tac.Semantic_Model
{
    ...
    internal class ExteranlResolvableScope : IResolvableScope
    {
        ...
        public bool TryGetType(IKey key, out IBox<IIsPossibly<IFrontendType<IVerifiableType>>> res)
        {
            ...
            if (genericType is IGenericInterfaceDefinition genericInterfaceDefinition)
            {
                res = new Box<IIsPossibly<IFrontendType<IVerifiableType>>>(Possibly.Is(genericTypeCache.GetOrAdd(genericType.Type, new ExternalGenericType(generic, genericInterfaceDefinition.Type))));
                return true;
            }
            else {
