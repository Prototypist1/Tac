...
namespace Tac.Frontend.New.CrzayNamespace
{
    ...
    {
        internal abstract class TypeRequirementWithExternal : OrType<
            MustHave, 
            MustBePrimitive, 
            GivenPathThen, 
            HasMembers, 
            ...
            IsExternal>
        {
            public static TypeRequirementWithExternal Make(MustHave contents)
            {
                return new Inner<MustHave>(contents);
            }
            public static TypeRequirementWithExternal Make(MustBePrimitive contents)
            {
                return new Inner<MustBePrimitive>(contents);
            }
            public static TypeRequirementWithExternal Make(GivenPathThen contents)
            {
                return new Inner<GivenPathThen>(contents);
            }
            public static TypeRequirementWithExternal Make(HasMembers contents)
            {
                return new Inner<HasMembers>(contents);
            }
            public static TypeRequirementWithExternal Make(IsGeneric contents)
            {
                return new Inner<IsGeneric>(contents);
            }
            public static TypeRequirementWithExternal Make(IsExternal contents)
            {
                return new Inner<IsExternal>(contents);
            }
            public static TypeRequirementWithExternal Make(IError contents)
            {
                return new Inner<IError>(contents);
            }

            private class Inner<T> : TypeRequirementWithExternal, IIsDefinately<T>
            {
