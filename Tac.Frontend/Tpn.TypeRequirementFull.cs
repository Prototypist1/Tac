...
namespace Tac.Frontend.New.CrzayNamespace
{
    ...
    {
        internal abstract class TypeRequirementFull : OrType<
           MustHave,
           MustBePrimitive,
           GivenPathThen,
           OrConstraint,
           ...
           IsExternal>
        {
            public static TypeRequirementFull Make(MustHave contents)
            {
                return new Inner<MustHave>(contents);
            }
            public static TypeRequirementFull Make(MustBePrimitive contents)
            {
                return new Inner<MustBePrimitive>(contents);
            }
            public static TypeRequirementFull Make(GivenPathThen contents)
            {
                return new Inner<GivenPathThen>(contents);
            }
            public static TypeRequirementFull Make(OrConstraint contents)
            {
                return new Inner<OrConstraint>(contents);
            }
            public static TypeRequirementFull Make(HasMembers contents)
            {
                return new Inner<HasMembers>(contents);
            }
            public static TypeRequirementFull Make(IsGeneric contents)
            {
                return new Inner<IsGeneric>(contents);
            }
            public static TypeRequirementFull Make(IsExternal contents)
            {
                return new Inner<IsExternal>(contents);
            }

            public static TypeRequirementFull Make(IError contents)
            {
                return new Inner<IError>(contents);
            }

            private class Inner<T> : TypeRequirementFull, IIsDefinately<T>
            {
